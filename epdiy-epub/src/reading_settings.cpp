...
/*==========================================================================
 * 辅助：截断 UTF-8 字符串到指定最大字节数（不截断半个 UTF-8 字符）
 * dst 缓冲区至少需要 max_bytes + 3 字节（截断时追加 ".."）
 *========================================================================*/
...
void reading_settings_draw(Renderer *renderer)
{
    ...
    /* 值列起始 x：屏幕 40% 处；值字符串截断阈值 */
    int x_value = page_w * 2 / 5;
    int max_name_bytes = 20;  /* 字体名最多显示 20 字节（约 6-7 个汉字） */

    /* ---- 标题居中 ---- */
    const char *title = "\xe9\x98\x85\xe8\xaf\xbb\xe8\xae\xbe\xe7\xbd\xae";  /* 阅读设置 */
    int title_w = epd_font_ft_get_size() * 4;  /* 4 个汉字粗略估算 */
    ...
    for (int i = 0; i < SETTING_COUNT; i++) {
        /* 防止大字号时设置项超出屏幕（底部留 4 行给操作提示） */
        if (y + line_h > page_h - line_h * 4) break;
    ...
    /* ---- 操作提示（锚定到底部） ---- */
    y = page_h - line_h * 4;
    if (y < page_h / 2) y = page_h / 2;  /* 防止和设置项重叠 */
    ...
    renderer->draw_text(x_label, y,
        "\xe4\xb8\x8a/\xe4\xb8\x8b: \xe8\xb0\x83\xe6\x95\xb4\xe5\x80\xbc",
        false, false);
    y += line_h;
    ...
    if (y + line_h <= page_h) {
        /* 确认: 下一项 */
        renderer->draw_text(x_label, y,
            "\xe7\xa1\xae\xe8\xae\xa4: \xe4\xb8\x8b\xe4\xb8\x80\xe9\xa1\xb9",
            false, false);
        y += line_h;
    }
    ...
    if (y + line_h <= page_h) {
        /* 长按K2: 保存并退出 */
        renderer->draw_text(x_label, y,
            "\xe9\x95\xbf\xe6\x8c\x89K2: \xe4\xbf\x9d\xe5\xad\x98\xe5\xb9\xb6\xe9\x80\x80\xe5\x87\xba",
            false, false);
    }
...
bool reading_settings_handle_action(Renderer *renderer, UIAction action)
{
    ...
    case UP:
        ...
        case SETTING_FONT_TYPE:
            if (g_font_type_idx < font_count - 1) {
                g_font_type_idx++;
            ...
            break;
        case SETTING_FONT_SIZE:
            if (g_font_size_idx < FONT_SIZES_COUNT - 1) {
                g_font_size_idx++;
                g_settings_changed = true;
            }
            break;
        case SETTING_BOLD:
            if (g_bold_idx < BOLD_LEVELS_COUNT - 1) {
                g_bold_idx++;
                g_settings_changed = true;
            }
            break;
        case SETTING_LINE_SPACING:
            if (g_line_spacing_idx < LINE_SPACINGS_COUNT - 1) {
                g_line_spacing_idx++;
                g_settings_changed = true;
            }
            break;
        case SETTING_MARGIN:
            if (g_margin_idx < MARGINS_COUNT - 1) {
                g_margin_idx++;
                g_settings_changed = true;
            }
            break;
    ...
    case SELECT:
        g_current_item = (SettingItem)((g_current_item + 1) % SETTING_COUNT);
        reading_settings_draw(renderer);
        return true;
