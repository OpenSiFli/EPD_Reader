...
bool Epub::find_content_opf_file(ZipFile &zip, std::string &content_opf_file)
{
  ...
  if (!meta_info)
  {
    ulog_e(TAG, "Could not find META-INF/container.xml");
    return false;
  }
  // parse the meta data
  ...
  if (!container)
  {
    ulog_e(TAG, "Could not find container element in META-INF/container.xml");
    return false;
  }
  auto rootfiles = container->FirstChildElement("rootfiles");
  if (!rootfiles)
  {
    ulog_e(TAG, "Could not find rootfiles element in META-INF/container.xml");
    return false;
  }
  ...
  ulog_e(TAG, "Could not get path to content.opf file");
  return false;
}
...
bool Epub::parse_content_opf(ZipFile &zip, std::string &content_opf_file)
{
  ...
  if (!package)
  {
    ulog_e(TAG, "Could not find package element in content.opf");
    return false;
  }
  ...
  if (!metadata)
  {
    ulog_e(TAG, "Missing metadata");
    return false;
  }
  auto title = metadata->FirstChildElement("dc:title");
  if (!title)
  {
    ulog_e(TAG, "Missing title");
    return false;
  }
...
bool Epub::parse_toc_ncx_file(ZipFile &zip)
{
  ...
  {
    ulog_e(TAG, "No ncx file specified");
    return false;
  }
  ulog_i(TAG, "toc path: %s\n", m_toc_ncx_item.c_str());

  char *ncx_data = (char *)zip.read_file_to_memory(m_toc_ncx_item.c_str());
  if (!ncx_data)
  {
    ulog_e(TAG, "Could not find %s", m_toc_ncx_item.c_str());
    return false;
  }
  ...
  if (!ncx)
  {
    ulog_e(TAG, "Could not find first child ncx in toc");
    return false;
  }
  ...
  if (!navMap)
  {
    ulog_e(TAG, "Could not find navMap child in ncx");
    return false;
  }
  ...
  while (navPoint)
  {
    ...
    // ulog_i(TAG, "%s -> %s#%s", title.c_str(), href.c_str(), anchor.c_str());
...
bool Epub::load()
{
  ...
  if (!find_content_opf_file(zip, content_opf_file))
  {
    #ifndef UNIT_TEST
      #ifdef USE_SPIFFS
        ulog_i(TAG, "Is SPIFFs data uploaded?\npio run -t uploadfs");
      #endif
      ulog_e(TAG, "Could not open ePub. Restarting in 10 secs.");
...
uint8_t *Epub::get_item_contents(const std::string &item_href, size_t *size)
{
  ...
  if (!content)
  {
    ulog_e(TAG, "Failed to read item %s", path.c_str());
    return nullptr;
  }
  return content;
}
...
std::string &Epub::get_spine_item(int spine_index)
{
  ...
  if(spine_index < m_spine.size())
  {
      return m_spine.at(spine_index).second;
  }
  else
  {
      ulog_i(TAG, "get_spine_item index:%d is out_of_range", spine_index);
      spine_index = 0;
      return m_spine.at(spine_index).second;
  }
#else
