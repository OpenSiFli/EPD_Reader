...
bool JPEGHelper::get_size(const uint8_t *data, size_t data_size, int *width, int *height)
{
  ...
  if (res == JDR_OK)
  {
    ulog_i(TAG, "JPEG Decoded - size %d,%d", dec.width, dec.height);
    *width = dec.width;
    *height = dec.height;
  }
  else
...
bool JPEGHelper::render(const uint8_t *data, size_t data_size, Renderer *renderer, int x_pos, int y_pos, int width, int height)
{
  ...
  if (res == JDR_OK)
  {
    ...
    ulog_i(TAG, "JPEG Decoded - size %d,%d, scale = %f, %f, %d", dec.width, dec.height, x_scale, y_scale, scale_factor);
