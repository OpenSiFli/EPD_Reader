...
bool PNGHelper::get_size(const uint8_t *data, size_t data_size, int *width, int *height)
{
  ...
  if (rc == PNG_SUCCESS)
  {
    ulog_i(TAG, "image specs: (%d x %d), %d bpp, pixel type: %d", png.getWidth(), png.getHeight(), png.getBpp(), png.getPixelType());
    *width = png.getWidth();
  ...
  {
    ulog_e(TAG, "failed to open png %d", rc);
    return false;
  }
}
bool PNGHelper::render(const uint8_t *data, size_t data_size, Renderer *renderer, int x_pos, int y_pos, int width, int height)
{
  ...
  {
    ulog_e(TAG, "failed to parse png %d", rc);
    return false;
  }
}
